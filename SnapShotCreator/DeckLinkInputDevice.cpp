...
HRESULT DeckLinkInputDevice::Init()
{
	...
	if (result != S_OK)
	{
		logger.e("Unable to get IDeckLinkInput interface");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		logger.e("Unable to get IDeckLinkDisplayModeIterator interface");
		goto bail;
	}
...
HRESULT DeckLinkInputDevice::StartCapture(BMDDisplayMode displayMode, BMDPixelFormat pixelFormat, bool enableFormatDetection)
{
	...
	if (result != S_OK)
	{
		logger.e("Unable to enable video input. Perhaps, the selected device is currently in-use.");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		logger.e("Unable to start input streams");
		goto bail;
	}

	logger.d("Capture started");
...
void DeckLinkInputDevice::StopCapture()
{
	...
	{
		logger.e("DeckLinkInputDevice not defined");
	}
}
...
HRESULT DeckLinkInputDevice::VideoInputFormatChanged(/* in */ BMDVideoInputFormatChangedEvents notificationEvents, /* in */ IDeckLinkDisplayMode *newMode, /* in */ BMDDetectedVideoInputFormatFlags detectedSignalFlags)
{
	...
	if (result != S_OK)
	{
		logger.e("Unable to re-enable video input on auto-format detection");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		logger.e("Unable to restart streams on auto-format detection");
		goto bail;
	}
	...
	if (result == S_OK)
	{
		logger.d("Video format changed to ", DlToCString(displayModeNameStr), " ", (detectedSignalFlags & bmdDetectedVideoInputRGB444) ? "RGB" : "YUV");
		DeleteString(displayModeNameStr);
	}
	else
		logger.e("Unable to get new video format name");
