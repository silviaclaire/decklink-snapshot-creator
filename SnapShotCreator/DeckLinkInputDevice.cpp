...
HRESULT DeckLinkInputDevice::Init()
{
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to get IDeckLinkInput interface");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to get IDeckLinkDisplayModeIterator interface");
		goto bail;
	}
...
HRESULT DeckLinkInputDevice::StartCapture(BMDDisplayMode displayMode, BMDPixelFormat pixelFormat, bool enableFormatDetection)
{
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to enable video input. Perhaps, the selected device is currently in-use.");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to start input streams");
		goto bail;
	}

	spdlog::debug("Capture started");
...
void DeckLinkInputDevice::StopCapture()
{
	...
	{
		spdlog::error("DeckLinkInputDevice not defined");
	}
}
...
HRESULT DeckLinkInputDevice::VideoInputFormatChanged(/* in */ BMDVideoInputFormatChangedEvents notificationEvents, /* in */ IDeckLinkDisplayMode *newMode, /* in */ BMDDetectedVideoInputFormatFlags detectedSignalFlags)
{
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to re-enable video input on auto-format detection");
		goto bail;
	}
	...
	if (result != S_OK)
	{
		spdlog::error("Unable to restart streams on auto-format detection");
		goto bail;
	}
	...
	if (result == S_OK)
	{
		...
		spdlog::error("Unable to get new video format name");
